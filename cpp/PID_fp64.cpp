...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        double prevX = (double) inputs[2][0];
        double prevE = (double) inputs[3][0];
        double prevP = (double) inputs[4][0];
        double prevI = (double) inputs[5][0];
        double prevD = (double) inputs[6][0];
        double Kp = (double) inputs[7][0];
        double Ki = (double) inputs[8][0];
        double Kd = (double) inputs[9][0];
        double PID;
        PIDController<double>(setpoint, dt, prevX,
             prevE, prevP, prevI, prevD, 
             Kp, Ki, Kd, PID);
        outputs[0] = factory.createScalar(prevE);
        outputs[1] = factory.createScalar(prevP);
        outputs[2] = factory.createScalar(prevI);
        outputs[3] = factory.createScalar(prevD);
