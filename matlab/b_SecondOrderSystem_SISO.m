function b_SecondOrderSystem_SISO
...
% s = tf('s','ts',dt);
s = tf('s');
G = 1/((s+1)*(0.1*s+1)); % transfer function
% G = (10/9)/(s+1)+(-1/9)/(0.1*s+1); % transfer function
f1 = @(v) 1/((v+1)*(0.1*v+1));
f2 = @(v) (10/9)/(v+1)+(-10/9)/(v+10);
f3 = @(v) (1/(v+1))*(10/(v+10));
num = randn();
...
for i = 1:n
    ...
    % Plant output G(s)=1/((s+1)(0.1s+1))
    % parallel
%     X_Dot1(i+1) = -X1(i) + (10/9)*U(i);  
%     X_Dot2(i+1) = -10*X2(i) + (-10/9)*U(i);  
%     Y(i+1) = X_Dot1(i+1) + X_Dot2(i+1);
    % sequential
    X_Dot1(i+1) = -X1(i) + U(i);  
