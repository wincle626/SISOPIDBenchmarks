function h_UnstablePole_SISO

% The Multiple Equal Poles benchmark
% Transfer function: G(s) = 1/(s^2-1)
...
for i = 1:n
    ...
    % Plant output G(s) = 1/(s^2-1)
%     % parallel
    X1_Dot(i+1) = -X1(i)-0.5*U(i);
    X2_Dot(i+1) = X2(i)+0.5*U(i);
    Y(i+1) = X1_Dot(i+1)+X2_Dot(i+1);
    % sequential
%     X1_Dot(i+1) = -X1(i)+U(i);
